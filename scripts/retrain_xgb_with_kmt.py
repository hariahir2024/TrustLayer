"""
BehaviorShield — XGBoost Retraining Script using KMT Dataset
...
def map_kmt_key_events(kmt_events):
    """Map raw KMT key pressed/released events to BehaviorShield format."""
    mapped = []
    key_to_pos = {}
    pos_counter = 1
...
def map_kmt_mouse_events(kmt_mouse):
    """Map raw KMT mouse movement and click events to BehaviorShield format."""
    mapped = []
    for ev in kmt_mouse:
        event_type = ev.get("Event") or ev.get("event")
        epoch_val = ev.get("Epoch") or ev.get("epoch")
...
def main():
    log.info("BehaviorShield XGBoost Retraining with KMT Dataset")
    log.info("=" * 60)
    ...
    for filename in user_files:
        ...
        for attempt in range(6, 11):
            ...
            vector = [k_score, m_score, metadata_score, dev_match, tod_risk, 1]
    ...
    X_syn = np.column_stack([k_scores, m_scores, md_scores, dev_matches, tod_risks, enrollments])
    ...
    # Save metadata
    feature_names = ["keystroke_score", "mouse_score", "metadata_score", "device_match", "time_of_day_risk", "is_enrolled"]
    ...
    with open("scripts/retrain_results_kmt.txt", "w") as f:
        f.write("BehaviorShield KMT Retraining Report\n")
        f.write("=" * 60 + "\n")
